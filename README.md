...
| 故障表现                                | 原因                                    | 排查方法                                 |
| --------------------------------------- | --------------------------------------- | ---------------------------------------- |
| 串口屏主页飞镖索引长时间显示-1          | 通信异常，串口屏无法接收到C板发送的数据 | 检查开发板到串口屏之间的连线             |
| 串口屏补偿量调整页面所有数据显示99999   | 通信异常，串口屏无法接收到C板发送的数据 | 检查开发板到串口屏之间的连线             |
| 串口屏白屏，红字显示stackbot Error..... | 监视数据勾选过多，串口屏爆栈            | 重启整个系统或拔插UART连线单独重启串口屏 |
...
通信方式为串口，配置为：波特率115200，8位数据位，1位停止位，无硬件流控，无校验位。
...
| cmd_id       | 1           | 命令ID                     |
| vavid_data   | n           | 有效数据                   |
| frame_tail   | 2           | 帧尾，CRC16 MODBUS整包校验 |

frame_header结构：

| 结构        | 大小(bytes) | 说明                         |
| ----------- | ----------- | ---------------------------- |
| SOF         | 1           | 数据帧起始字节，固定值为0xA5 |
| data_length | 1           | 数据帧中vavid_data的长度     |

cmd_id功能映射：

| cmd_id | enum 名称                   | 数据段长度 | 说明                                 |
| ------ | --------------------------- | ---------- | ------------------------------------ |
| 7      | ID_outpost_speed_1          | 1~5        | 前哨站转速1，ASCII字符串，int16      |
| 8      | ID_outpost_speed_2          | 1~5        | 前哨站转速2，ASCII字符串，int16      |
| 9      | ID_outpost_speed_3          | 1~5        | 前哨站转速3，ASCII字符串，int16      |
| 10     | ID_outpost_speed_4          | 1~5        | 前哨站转速4，ASCII字符串，int16      |
| 11     | ID_base_speed_1             | 1~5        | 基地转速1，ASCII字符串，int16        |
| 12     | ID_base_speed_2             | 1~5        | 基地转速2，ASCII字符串，int16        |
| 13     | ID_base_speed_3             | 1~5        | 基地转速3，ASCII字符串，int16        |
| 14     | ID_base_speed_4             | 1~5        | 基地转速4，ASCII字符串，int16        |
| 15     | ID_outpost_yaw_offset_num_1 | 1~24       | 前哨站YAW补偿量1，ASCII字符串，float |
| 16     | ID_outpost_yaw_offset_num_2 | 1~24       | 前哨站YAW补偿量2，ASCII字符串，float |
| 17     | ID_outpost_yaw_offset_num_3 | 1~24       | 前哨站YAW补偿量3，ASCII字符串，float |
| 18     | ID_outpost_yaw_offset_num_4 | 1~24       | 前哨站YAW补偿量4，ASCII字符串，float |
| 19     | ID_base_yaw_offset_num_1    | 1~24       | 基地YAW补偿量1，ASCII字符串，float   |
| 20     | ID_base_yaw_offset_num_2    | 1~24       | 基地YAW补偿量2，ASCII字符串，float   |
| 21     | ID_base_yaw_offset_num_3    | 1~24       | 基地YAW补偿量3，ASCII字符串，float   |
| 22     | ID_base_yaw_offset_num_4    | 1~24       | 基地YAW补偿量4，ASCII字符串，float   |
| 100    | ID_refresh                  | 0          | 刷新串口屏数据                       |
| 101    | ID_fric_monitor             | 2          | 摩擦轮监控，16位复选框               |
| 102    | ID_other_monitor            | 2          | 其他数据监控，15位复选框             |
| 103    | ID_main_page_message        | 0          | 主页信息                             |
| 104    | ID_warning_message          | 0          | 警告信息                             |

7~22为参数设置类ID，串口屏向MCU发送数据，改变MCU内的参数。100~104为数据请求类ID，MCU收到后向串口屏返回请求的数据。

转速类数据的数据长度受到转速数据类型int16_t的限制，最大为16384即5个ASCII字符。YAW补偿量类数据受到串口屏文本控件txt_maxl属性限制，最大为24个字符（可修改），但设置时精确到小数点后两位即可满足打击前哨站和基地的精度需求。

转速和YAW补偿类cmd_id从7开始是受到串口屏offset页面各个文本控件ID的影响，用于装载参数的文本控件ID从7开始依次递增，将7作为cmd_id最小值便于编写串口屏的程序。
...
| 结构       | 大小(bytes) | 说明                                 |
| ---------- | ----------- | ------------------------------------ |
| 指令       | n           | 对串口屏的操作，语法见淘晶驰资料中心 |
| frame_tail | 3           | 结束符，三个0xFF                     |

代码中已将发送函数封装为`void TJCPrintf(const char *str, ...)`，调用函数即可实现追加帧尾和发送的功能。

例如main页面有一个文本控件t0，要让t0显示“TUP”，则通过单片机发送main.t0.txt="TUP"，然后发送3个0xFF作为结束符，函数调用为：`TJCPrintf("main.t0.txt=\"TUP\"")`
...
[淘晶驰资料中心 — 淘晶驰串口屏资料中心](http://wiki.tjc1688.com/)