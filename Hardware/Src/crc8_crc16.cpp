...
uint16_t CRC16_MODBUS_INIT = 0xffff;
const unsigned int CRC16_Modbus_table[256] =
{
 ...
 * @retval         计算完的CRC16
 */
uint16_t get_CRC16_MODBUS_check_sum(uint8_t *pch_message, uint32_t dw_length, uint16_t wCRC)
{
  while (dw_length--)
  {
    wCRC = (wCRC >> 8) ^ CRC16_Modbus_table[(wCRC ^ *pch_message++) & 0xff];
  }
 ...
 * @retval         真或者假
 */
uint32_t verify_CRC16_MODBUS_check_sum(uint8_t *pchMessage, uint32_t dwLength)
{
  uint16_t wExpected = 0;
  if ((pchMessage == NULL) || (dwLength <= 2))
  {
  ...
  wExpected = get_CRC16_MODBUS_check_sum(pchMessage, dwLength - 2, CRC16_MODBUS_INIT);
  return ((wExpected & 0xff) == pchMessage[dwLength - 2] && ((wExpected >> 8) & 0xff) == pchMessage[dwLength - 1]);
}
