 ...
 * @retval         计算完的CRC16
 */
extern uint16_t get_CRC16_MODBUS_check_sum(uint8_t *pch_message, uint32_t dw_length, uint16_t wCRC);

/**
 * @brief          CRC16 MODBUS校验函数
 * @param[in]      pch_message: 数据
 * @param[in]      dw_length: 数据和校验的长度
 * @retval         真或者假
 */
extern uint32_t verify_CRC16_MODBUS_check_sum(uint8_t *pchMessage, uint32_t dwLength);
